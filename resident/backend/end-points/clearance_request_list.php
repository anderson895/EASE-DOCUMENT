...
if ($fetch_all_clearance_request): ?>
    <?php foreach ($fetch_all_clearance_request as $request):
       if($request['rcl_status']=='Pending'){
        ...
        ?>
      <tr>
    <td class="p-2"><?= $request['rcl_id']; ?></td>
    <td class="p-2"><?= $request['rcl_purpose']; ?></td>
    <td class="p-2"><?= $request['rcl_address']; ?></td>
    <td class="p-2"><?= $request['rcl_payment']; ?></td>
    <td class="p-2">
        <?= (new DateTime($request['rcl_request_date']))->format('F j, Y, g:i A'); ?>
    </td>

    <td class="p-2"><?= $request['rcl_status']; ?></td>
    <td class="p-2">
